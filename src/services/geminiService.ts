...
export async function* runSentimentStream(asset: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Analiza el sentimiento actual del mercado para: ${asset}. ${getCurrentDateContext()}\nBusca en X.com, Reddit y titulares de noticias recientes. ¿Hay euforia, pánico o indiferencia? Resume los temas de conversación principales en 3 viñetas.`;
  const sys = "Eres un analista de sentimiento de mercado. Tu trabajo es medir la psicología de las masas.";
  yield* streamLLM(prompt, sys, true);
}

export async function* runFundamentalsStream(asset: string, financialContext: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Analiza los fundamentales duros de: ${asset}. Contexto: ${financialContext} ${getCurrentDateContext()}\nRevisa ingresos, deuda, P/E ratio (si es acción) o tokenomics/adopción (si es crypto). Resume la salud financiera en 3 viñetas.`;
  const sys = "Eres un analista fundamental purista. Solo te importan los números, balances y ventajas competitivas.";
  yield* streamLLM(prompt, sys, true);
}

export async function* runTechnicalsStream(asset: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Analiza la acción del precio puramente técnica para: ${asset}. ${getCurrentDateContext()}\nRevisa soportes/resistencias clave, RSI, MACD y medias móviles. Resume la estructura del gráfico en 3 viñetas.`;
  const sys = "Eres un analista técnico (Chartista). Tu mundo son las velas japonesas y los indicadores matemáticos de precio.";
  yield* streamLLM(prompt, sys, true);
}
...
export async function* runLiquidityStream(asset: string): AsyncGenerator<string, void, unknown> {
  ...
  const sys = "Eres un rastreador de liquidez institucional. Sigues el rastro del 'Smart Money' a través de derivados y datos on-chain.";
  yield* streamLLM(prompt, sys, true);
}

export async function* runMonetaryStream(asset: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Analiza el entorno monetario actual y su impacto en: ${asset}. ${getCurrentDateContext()}\nRevisa la postura de la FED/BCE, tasas de interés, inflación (CPI) y liquidez global (M2). Resume en 3 viñetas.`;
  const sys = "Eres un experto en bancos centrales y política monetaria. Entiendes que la liquidez mueve los mercados.";
  yield* streamLLM(prompt, sys, true);
}

export async function* runGeopoliticsStream(asset: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Analiza el riesgo geopolítico actual y su impacto en: ${asset}. ${getCurrentDateContext()}\nRevisa conflictos globales, guerras comerciales, aranceles, elecciones y cadenas de suministro. Resume en 3 viñetas.`;
  const sys = "Eres un analista de inteligencia geopolítica. Evalúas cómo los eventos del mundo real afectan a los activos financieros.";
  yield* streamLLM(prompt, sys, true);
}
...
export async function* runLeadResearcherStream(asset: string, sentiment: string, fundamentals: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Como Jefe de Investigación Fundamental, sintetiza los reportes de tus sub-agentes para ${asset}:\n\nSENTIMIENTO:\n${sentiment}\n\nFUNDAMENTALES:\n${fundamentals}\n\nEmite un veredicto consolidado sobre el valor intrínseco y la percepción pública del activo.`;
  const sys = "Eres el Lead Fundamental Analyst. Tu trabajo es encontrar divergencias entre el valor real (fundamentales) y el precio (sentimiento).";
  yield* streamLLM(prompt, sys, false);
}
...
export async function* runLeadQuantStream(asset: string, technicals: string, liquidity: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Como Jefe Cuantitativo, sintetiza los reportes de tus sub-agentes para ${asset}:\n\nTÉCNICO:\n${technicals}\n\nLIQUIDEZ/FLUJOS:\n${liquidity}\n\nEmite un veredicto consolidado sobre la dirección más probable del precio a corto/medio plazo basado en la estructura y los flujos.`;
  const sys = "Eres el Lead Quant. Combinas la acción del precio con el posicionamiento institucional para predecir el próximo movimiento.";
  yield* streamLLM(prompt, sys, false);
}
...
export async function* runLeadMacroStream(asset: string, monetary: string, geopolitics: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Como Jefe de Estrategia Macro, sintetiza los reportes de tus sub-agentes para ${asset}:\n\nMONETARIO:\n${monetary}\n\nGEOPOLÍTICA:\n${geopolitics}\n\nEmite un veredicto consolidado sobre si el entorno global es un viento a favor o en contra para este activo.`;
  const sys = "Eres el Lead Macro Strategist. Tu visión es 'Top-Down', evaluando si el ecosistema global permite que este activo prospere.";
  yield* streamLLM(prompt, sys, false);
}
...
export async function* runRiskManagerStream(asset: string, leadRes: string, leadQuant: string, leadMacro: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Realiza un "Stress Test" para ${asset} basado en los reportes de los Jefes de Departamento:\n\nFUNDAMENTAL:\n${leadRes}\n\nQUANT:\n${leadQuant}\n\nMACRO:\n${leadMacro}\n\nActúa como el Abogado del Diablo. ¿Qué podría salir mal? Identifica riesgos de cola y da un Nivel de Riesgo (Bajo, Medio, Alto, Extremo).`;
  const sys = "Eres el Chief Risk Officer (CRO). Tu único trabajo es proteger el capital, ser paranoico y buscar fallas en las tesis alcistas.";
  yield* streamLLM(prompt, sys, false);
}
...
export async function* runCommitteeDebateStream(asset: string, leadRes: string, leadQuant: string, leadMacro: string, risk: string): AsyncGenerator<string, void, unknown> {
  const prompt = `Simula una mesa redonda (debate) final y concisa entre los Jefes de Departamento sobre el activo: ${asset}.
    ${getCurrentDateContext()}
  ...
  const sys = "Eres el moderador del Comité de Inversiones. Tu objetivo es forzar a los especialistas a debatir sus diferencias y llegar a un consenso unificado antes de pasarle el reporte al CIO.";
  yield* streamLLM(prompt, sys, false);
}
